...
class TradingExecutor:
    def __init__(self):
        ...
        self.symbol_id = None  # Will store EURUSD symbol ID
    ...
    def on_message_received(self, client, message):
        ...
        if message.payloadType == ProtoOAGetAccountListByAccessTokenRes().payloadType:
            ...
            # Only print EURUSD symbol details when found
            for symbol in msg.symbol:
                if symbol.symbolName == "EURUSD":
                    self.symbol_id = symbol.symbolId
                    print(f"\nFound EURUSD symbol ID: {self.symbol_id}")
                    self.get_market_price()  # Get market price instead of immediate trade
                    break
        # Handle spot events only if no order is placed yet
        if message.payloadType == ProtoOASpotEvent().payloadType and not self.order_placed:
    ...
    def get_market_price(self):
        """Get current market price for EURUSD"""
        print("\nGetting current market price for EURUSD...")  # Add clear messaging
        
        # Use ProtoOASpotEvent instead of ProtoOASymbolByIdReq
    ...
    def on_symbol_info_received(self, response):
        """Handle symbol info response and get current price"""
        if hasattr(response, 'bid') and hasattr(response, 'ask'):
            self.current_market_price = (response.bid + response.ask) / 2
    ...
    def get_order_input(self):
        ...
        print("\n=== Order Entry ===")
        print(f"Current market price: {self.current_market_price:.5f}")
        print("Please enter your target price for EURUSD (close to current price, e.g. 1.05000):")
    ...
    def place_trade(self, order_type, price, trade_side):
        ...
        #if input price > curent price, order type is buy stop, else buy limit
        ...
        #    order_req.stopPrice = price_int
        #elif order_type == ProtoOAOrderType.LIMIT:
        #    order_req.limitPrice = price_int
    ...
    def place_immediate_trade(self):
        """Place immediate EURUSD trade with SL and TP"""
        if self.order_placed:
            print("\nError: Order already placed")
            return
        
        if not self.symbol_id:
            print("\nError: EURUSD symbol ID not found")
            return

        # Define SL and TP in pips (for EURUSD, 1 pip = 0.0001)
        stop_loss_pips = 2.5  # 1.25 pips stop loss
        take_profit_pips = 5.0  # 2.50 pips take profit
        
        # Convert pips to price values
        pip_value = 0.0001  # For EURUSD
        stop_loss_value = stop_loss_pips * pip_value
        take_profit_value = take_profit_pips * pip_value

        print("\nPlacing EURUSD trade...")
        order_req = ProtoOANewOrderReq()
        order_req.ctidTraderAccountId = self.account_id
        order_req.symbolId = self.symbol_id
        order_req.orderType = ProtoOAOrderType.MARKET
        order_req.tradeSide = ProtoOATradeSide.BUY
        order_req.volume = 1000000  # Setting volume to 1000.00
        
        # For BUY orders:
        # SL is entry_price - stop_loss_value
        # TP is entry_price + take_profit_value
        order_req.relativeStopLoss = int(stop_loss_pips * 100)  # Convert to points (1 pip = 100 points)
        order_req.relativeTakeProfit = int(take_profit_pips * 100)  # Convert to points
        
        order_req.comment = "Automated EURUSD trade with SL/TP"

        print(f"\nSending order request for EURUSD (SL: {stop_loss_pips} pips, TP: {take_profit_pips} pips)...")
        deferred = self.client.send(order_req)
        deferred.addCallbacks(self.on_order_response, self.on_error)
        deferred.addTimeout(10, reactor)
        self.order_placed = True  # Set the flag to prevent subsequent orders
