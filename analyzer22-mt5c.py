...
def fetch_data_with_fallback(symbol, start_time, end_time, min_candles=30):
    ...
    for timeframe, timeframe_name in timeframes:
        ...
        extended_start = start_time_utc - timedelta(hours=2)
        extended_end = end_time_utc + timedelta(hours=2)
        
        # Request specific fields including volume
        ...
        if rates is not None and len(rates) >= min_candles:
            ...
            # Ensure volume data is present and convert tick_volume if real_volume is not available
            ...
            # Check if we actually got volume data
            if df['volume'].any():
                print(f"Volume data successfully retrieved for {symbol}")
            else:
    ...
    # Final attempt with extended historical data
    print(f"Attempting to fetch maximum available historical data for {symbol}...")
    extended_start = start_time_utc - timedelta(days=30)
    extended_end = end_time_utc + timedelta(days=30)
    
    for timeframe, timeframe_name in reversed(timeframes):
        rates = mt5.copy_rates_range(symbol, timeframe, extended_start, extended_end)
        if rates is not None and len(rates) >= min_candles:
...
def plot_ohlc(df, symbol, event_time, event_data, atr_multiple, spread, atr_period=14,
    ...
    if df['volume'].any():
        volume_color = 'blue'  # Changed to match analyzer21.py
        volume_alpha = 0.3    # Changed to match analyzer21.py
    ...
    ax1.set_title(f'{symbol} Price Action - {event_time.strftime("%Y-%m-%d %H:%M")} GMT+2', fontweight='bold')
...
def process_event(event, date_time_inputs, event_data_list, event_symbols):
    ...
    for idx, date_str in enumerate(event_dates):
        ...
        try:
            ...
            # Extended time range
            start_time = event_time - timedelta(hours=2)  # Extended further #time #period #window
            end_time = event_time + timedelta(hours=2)    # Extended further #time #period #window
...
print("\n")
print("Please input MT5 time (without DST adjustments)):")
print("\n")
print("Format example:")
print("Event\tNFP")
print("Actual:\tEURUSD")
print("Time (MT5):\t13:30")
...
output_pptx_path = "/Users/pawan/Desktop/Quantwater Tech Investments/Research & Development/Research/Blog/Slides/updated_presentation4.pptx"
